...
// --- Flattened course-wide data model ---
type FlatNode = {
  node: LearningNode;
  globalIndex: number;
  topicIndex: number;
...
type FlatStatus = 'completed' | 'current' | 'locked';

// A node is locked if ANY earlier node across the whole course is not passed.
function getFlatStatus(node: FlatNode, flat: FlatNode[]): FlatStatus {
  if (node.node.progress?.passed) return 'completed';
  const anyEarlierNotPassed = flat.slice(0, node.globalIndex).some(n => !n.node.progress?.passed);
  if (anyEarlierNotPassed) return 'locked';
  return 'current';
}
...
export default function CoursePathScreen() {
  ...
  const loadPath = async () => {
    try {
      ...
      pathData.forEach((topic, tIdx) => {
        topic.nodes.forEach((node, nIdx) => {
          flattened.push({
            node,
            globalIndex,
            topicIndex: tIdx,
            topicTitle: topic.title,
            isFirstInTopic: nIdx === 0,
            isLastInTopic: nIdx === topic.nodes.length - 1,
          });
          globalIndex++;
        });
      });
      ...
      // Target node: if a topicId was passed (came from a topic card), scroll
      // to that topic's first node; otherwise open at the active (first unpassed) unit.
      let target: FlatNode | undefined;
      if (topicId) {
        target = flattened.find(f => String(f.node.topic) === String(topicId));
        // fall back to first node of the topic via topicIndex match
        if (!target) {
          const tIdx = pathData.findIndex(t => String(t.id) === String(topicId));
          target = flattened.find(f => f.topicIndex === tIdx);
        }
      }
      if (!target) {
        target = flattened.find(n => !n.node.progress?.passed);
      }
      if (target) {
        setActiveNodeIndex(target.globalIndex);
        setTimeout(() => {
          scrollRef.current?.scrollTo({ y: Math.max(0, getNodeY(target!.globalIndex) - SCREEN_H / 2 + 40), animated: false });
        }, 120);
      } else if (flattened.length > 0) {
        setActiveNodeIndex(flattened.length - 1);
      }
    } catch {
      // ignore
    } finally {
  ...
  const handleScroll = (e: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    const center = offsetY + scrollViewH.current / 2;
    if (flat.length === 0) return;
    let nearest = 0;
    let bestDist = Infinity;
    for (let i = 0; i < flat.length; i++) {
      const d = Math.abs(getNodeY(i) - center);
      if (d < bestDist) { bestDist = d; nearest = i; }
    }
    setActiveNodeIndex(nearest);
  };

  const statuses = flat.map(n => getFlatStatus(n, flat));
  const completedCount = statuses.filter(s => s === 'completed').length;
  const progressPct = flat.length > 0 ? completedCount / flat.length : 0;
  const contentHeight = flat.length > 0 ? getNodeY(flat.length) + 100 : 400;
  ...
  return (
    <LinearGradient colors={[COLORS.bgTop, COLORS.bgMid, COLORS.bgBottom]} locations={[0, 0.32, 0.75]} style={styles.container}>
      ...
      >
        {flat.length === 0 && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySub}>Check back soon — this course is being built.</Text>
          </View>
        )}

        {/* Dotted trail connecting consecutive nodes along the path */}
        ...
        {flat.length > 0 && (
          <TrailSegment
            from={{ x: getNodeX(flat.length - 1), y: getNodeY(flat.length - 1) }}
            to={{ x: PATH_W / 2, y: getNodeY(flat.length) + 32 }}
            done={statuses[flat.length - 1] === 'completed'}
          />
        )}
        ...
        {flat.length > 0 && (
          <View style={[styles.trophyWrap, { left: PATH_W / 2 - 42, top: getNodeY(flat.length) }]}>
      ...
      {selectedNode && selectedNodeIndex !== null && (() => {
        const cfg = NODE_TYPE_CONFIG[selectedNode.node.node_type];
        ...
        const startLabel =
          st === 'completed' ? 'Review'
          : selectedNode.node.progress && !selectedNode.node.progress.passed ? 'Retry'
          : 'Start';
          ...
          <Pressable style={styles.overlay} onPress={() => setSelectedNodeIndex(null)}>
            ...
            >
              ...
              <Pressable onPress={(e) => e.stopPropagation()}>
                ...
                {selectedNode.isFirstInTopic && (
                  <View style={styles.topicChip}>
                    <Ionicons name="folder-open-outline" size={13} color={COLORS.purpleVibrant} />
                    <Text style={styles.topicChipText}>{selectedNode.topicTitle}</Text>
                  </View>
                )}
                ...
                <View style={styles.popupMeta}>
                  <View style={styles.metaItem}>
                    ...
                    <Text style={styles.metaText}>{selectedNode.node.estimated_minutes} min</Text>
                  </View>
                  <View style={styles.metaItem}>
                    <Ionicons name="star" size={14} color={COLORS.warning} />
                    <Text style={styles.metaText}>{selectedNode.node.xp_reward} XP</Text>
                  </View>
                  {selectedNode.node.progress?.score !== undefined && selectedNode.node.progress.score > 0 && (
                    <View style={[styles.scorePill, selectedNode.node.progress.passed ? styles.scorePillPass : styles.scorePillFail]}>
                      <Text style={[styles.scorePillText, { color: selectedNode.node.progress.passed ? COLORS.success : COLORS.warning }]}>
                        {selectedNode.node.progress.score}%
                      </Text>
                    </View>
                  )}
                </View>
                ...
                {st === 'locked' ? (
                  ...
                  <TouchableOpacity activeOpacity={0.9} onPress={() => handleStartActivity(selectedNode.node.id)}>
                    <LinearGradient
